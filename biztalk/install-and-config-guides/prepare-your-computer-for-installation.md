...
Específica para la instalación de los pasos, consulte: 
...
Cuando se instala IIS, además de las opciones predeterminadas, seleccione las siguientes:  
...
- **Características de .net framework 3.5**: .NET Framework 4.5 y .NET Framework 3.5 se pueden usar para desarrollar aplicaciones .net que implique BizTalk Adapter Pack. Por lo general, **características de .NET Framework 4.5** ya está instalado. Si va a usar .NET Framework 3.5 para crear aplicaciones en el equipo y, a continuación, **características de .net Framework 3.5** también se puede instalar.  
...
-   Si instala Visual Studio antes de instalar BizTalk Server y, a continuación, actualiza a Visual Studio Team Explorer, es posible que deba reparar la instalación de BizTalk Server desde el **Panel de Control** / **programas**  opción.  
...
-   Las herramientas de desarrollo de BizTalk Server se basan en Visual Studio. Como mínimo, debe instalar el componente de Microsoft Visual C#® .NET de Visual Studio antes de instalar el servidor BizTalk Server**SDK y herramientas de desarrollo**.  
  
-   Visual Studio es *no* necesario si va a instalar BizTalk Server en un equipo de producción (solo tiempo de ejecución), en la aplicación que no se requiere el desarrollo o depuración.  
...
##  <a name="BKMK_SQL"></a> Instalar SQL Server  
 ...
 Al instalar SQL Server, seleccione las siguientes características:  
...
- Cuando BizTalk Server y SQL Server están en equipos independientes, Distributed Transaction Coordinator (MS DTC) controla las transacciones entre los equipos. La característica AlwaysOn de SQL Server no admite transacciones MSDTC. No se admite la característica AlwaysOn de SQL Server.  
...
##  <a name="BKMK_BAMAlerts"></a> Alertas de BAM  
 Las alertas de BAM con SQL Server 2012 y versiones más recientes usar correo electrónico de base de datos en SQL Server. Las alertas de BAM con SQL Server 2008 R2 y versiones anteriores de usar SQL Notification Services. Antes de instalar o configurar las alertas de BAM, debe configurar los servicios de notificación o correo electrónico de base de datos en SQL Server.  
...
-   Correo electrónico de base de datos usa un servidor SMTP para enviar las alertas de BAM. Servidor SMTP se puede instalar localmente en el servidor BizTalk Server o en otro servidor IIS. En [Apéndice D: Crear el servidor SMTP](../install-and-config-guides/appendix-d-create-the-smtp-server.md) se enumeran los pasos necesarios para instalar y configurar un servidor SMTP.  
...
2. Descargue e instale el paquete de plataforma adecuado para los siguientes componentes:  
   ...
   - HYPERLINK "<http://download.microsoft.com/download/3/1/6/316FADB2-E703-4351-8E9C-E0B36D9D697E/sqlncli.msi>" X86 del paquete (sqlncli.msi)  
  
   - HYPERLINK "<http://download.microsoft.com/download/3/1/6/316FADB2-E703-4351-8E9C-E0B36D9D697E/sqlncli_x64.msi>" X64 del paquete (sqlncli_x64.msi)  
  
     **Colección de objetos de administración de Microsoft SQL Server 2005**  
  
   - HYPERLINK "<http://download.microsoft.com/download/3/1/6/316FADB2-E703-4351-8E9C-E0B36D9D697E/SQLServer2005_XMO.msi>" X86 del paquete (SQLServer2005_XMO.msi)  
  
   - HYPERLINK "<http://download.microsoft.com/download/3/1/6/316FADB2-E703-4351-8E9C-E0B36D9D697E/SQLServer2005_XMO_x64.msi>" X64 del paquete (SQLServer2005_XMO_x64.msi)  
  
     **Componentes de cliente de Microsoft SQL Server 2005 Notification Services**  
  
   - HYPERLINK "<http://download.microsoft.com/download/3/1/6/316FADB2-E703-4351-8E9C-E0B36D9D697E/SQLServer2005_NS.msi>" X86 del paquete (SQLServer2005_NS.msi)  
  
   - HYPERLINK "<http://download.microsoft.com/download/3/1/6/316FADB2-E703-4351-8E9C-E0B36D9D697E/SQLServer2005_NS_x64.msi>" X64 del paquete (SQLServer2005_NS_x64.msi)  
  
   **Notas**  
  
-   SQL Notification Services no tiene que configurarse; solo se instala en el servidor BizTalk Server.  
...
|                      Windows Vista SP1                       | Descarga disponible en [Windows Identity Foundation](http://www.microsoft.com/download/details.aspx?id=17331) HYPERLINK "<http://www.microsoft.com/download/details.aspx?id=17331>". |
  
 **Notas**  
  
-   Windows Identity Foundation (WIF) se requiere para el adaptador de SharePoint o SharePoint Online cuando se usa con SharePoint objeto modelo cliente (CSOM). Concretamente:  
...
-   [Apéndice B: instalar el adaptador de Microsoft SharePoint](../install-and-config-guides/appendix-b-install-the-microsoft-sharepoint-adapter.md) proporciona información específica sobre las opciones de instalación de SharePoint.  
...
-   Si va a instalar SharePoint, debe hacerlo antes de continuar con los restantes requisitos previos de BizTalk Server.  
 ...
 **Windows Server 2012** : [Windows Server 2012: cómo agregar una cuenta a un grupo de administradores Local](http://social.technet.microsoft.com/wiki/contents/articles/13436.windows-server-2012-how-to-add-an-account-to-a-local-administrator-group.aspx)  
...
-   Debe ser miembro del grupo Administradores local para instalar y configurar BizTalk Server.  

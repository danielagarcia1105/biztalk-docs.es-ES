...
1.  Agregar un nuevo vocabulario. Esto implica definir valor de constante de al menos un vocabulario. Esto establece un umbral de regla de negocios. También implica la definición de documento XML `Get` y `Set` elementos. Esto establece cómo [!INCLUDE[btsCoName](../../includes/btsconame-md.md)] [!INCLUDE[BTARN_CurrentVersion_abbrev](../../includes/btarn-currentversion-abbrev-md.md)] usa el umbral.  
  
2.  Agregar una nueva directiva. Esto implica la creación de una directiva, crear un conjunto de reglas y, a continuación, guardar, publicar e implementar la directiva.  
...
2.  Si el **Abrir almacén de reglas** abre el cuadro de diálogo, seleccione la **motor de reglas de BizTalk** base de datos que ha configurado en el servidor actual y, a continuación, haga clic en **Aceptar**.  
  
3.  En [!INCLUDE[btsCoName](../../includes/btsconame-md.md)] Compositor de reglas de negocios, en el panel Explorador de hechos, haga clic en **vocabularios**y, a continuación, haga clic en **Agregar nuevo vocabulario**.  
...
7.  En el **valor constante, rango de valores o conjunto de valores** página, en la **nombre de definición de** , escriba el nombre del valor de constante de vocabulario adecuado, como **máximo permitido de cantidad** y, a continuación, haga clic en **siguiente**.  
  
8.  En el **definir un valor constante** página, en la **campo de valor** , escriba el umbral y, a continuación, haga clic en **finalizar**.  
...
3.  En el **atributo o elemento de documento XML** página, en el cuadro de texto Nombre de definición, escriba un nombre para un **obtener** elemento.  
  
4.  Haga clic en **examinar**, desplácese a la ubicación del esquema que desea usar, seleccione el archivo de esquema y, a continuación, haga clic en **abiertos**.  
  
5.  Si el **seleccionar el nodo raíz** se abre la página, seleccione el nodo raíz para examinar.  
...
10. Repita estos pasos para definir una o varias `Set` operaciones, seleccionadas **realizar operación "Set"** para el **tipo de operación**.  
...
6.  En el panel Propiedades, establezca la **nombre** propiedad para el nombre de la regla desee y, a continuación, presione **ENTRAR**.  
  
7.  En el Compositor de reglas, en la **IF** panel, haga clic en **condiciones**y, a continuación, seleccione una condición lógica, si procede.  
  
8.  En el panel Explorador de hechos, en **vocabularios**, expanda **predicados**, expanda **versión 1.0 - publicada**, seleccione el predicado que desee, arrástrelo hasta la superficie del compositor, y, a continuación, colóquela en **condiciones** o el operador lógico.  
  
9. En el panel Explorador de hechos, en la carpeta vocabularios, expanda el vocabulario que creó, **versión 1.0 - publicada**, seleccione un `Get` o `Set` elemento, arrástrelo hasta la superficie del compositor y colóquela en **argumento1**.  
  
10. En la carpeta de vocabulario, seleccione un `Get` o `Set` elemento, arrástrelo hasta la superficie del compositor y colóquela en **argumento2**.  
  
11. En la carpeta de vocabulario, seleccione un `Set` elemento, arrástrelo hasta la superficie del compositor y colóquela en la **acciones** cuadro en el panel entonces.  
  
12. Si una variable está asociada a la `Set` elemento, haga clic en la variable, realizar cambios según corresponda y, a continuación, presione **ENTRAR**. Si es necesario, repita con otros `Set` elementos.  
...
3.  En el panel Explorador de directivas, en la carpeta de directiva que ha creado, haga clic en **versión 1.0**y, a continuación, haga clic en **implementar**.  
  
### <a name="to-call-the-business-rule-from-the-orchestration"></a>Llamar a la regla de negocios desde la orquestación  
  
1.  Inicie **Microsoft Visual Studio 2012**.  
  
2.  En el **archivo** menú, seleccione Abrir y, a continuación, haga clic en **proyecto/solución**.  
  
3.  Busque la solución que contiene la orquestación que se debe llamar a la regla de negocios de y, a continuación, haga clic en **abiertos**.  
  
4.  Haga clic en **vista**, seleccione **otras ventanas**y, a continuación, haga clic en **Vista orquestación**.  
  
5.  Expanda **Variables**. Asegúrese de que la lista de variables de orquestación contiene una variable que corresponde a cada parámetro de la directiva empresarial que se llame desde la orquestación. Asegúrese de que la variable tiene el mismo tipo que el parámetro de directiva. Si la lista no contiene una variable de orquestación para cada parámetro de la directiva, haga clic en **Variables**y, a continuación, haga clic en **nueva Variable**. En la vista de orquestación, escriba un nombre de variable y, a continuación, en la ventana Propiedades, escriba el tipo del parámetro.  
  
6.  Desde el **cuadro de herramientas**, arrastre un **reglas de llamada** dar forma a la superficie de diseño de orquestación y colóquela en la **recepción** forma.  
  
7.  Haga doble clic en el **reglas de llamada** forma.  
  
8.  En el **seleccione la directiva empresarial que se va a llamar a** , seleccione la directiva empresarial en la lista desplegable.  
  
9. Para el primer parámetro que se muestra, para **nombre de parámetro**, seleccione un nombre de la lista desplegable.  
  
    > [!NOTE]
    >  BTARN rellena la **parámetro Policy** lista con todos los parámetros en la directiva empresarial. Para cada parámetro en la lista, BTARN entra en un valor en **tipo de parámetro** de la directiva empresarial. En la lista desplegable asociada **nombre de parámetro**, BTARN entra en los nombres de todas las variables de la lista de variables de orquestación que tenga el mismo tipo que los parámetros de directiva. Al seleccionar una variable de orquestación, va a asociar esa variable con el parámetro de directiva. Puede ver las variables de orquestación en Vista orquestación.  

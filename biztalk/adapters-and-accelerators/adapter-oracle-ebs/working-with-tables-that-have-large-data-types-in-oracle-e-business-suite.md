...
## <a name="how-this-topic-demonstrates-reading-and-writing-data-into-columns-of-large-data-types"></a>Cómo en este tema muestra cómo leer y escribir datos en columnas de tipos de datos de gran tamaño  
 Para mostrar leer y escribir datos en columnas de tipos de datos de gran tamaño, este tema proporciona instrucciones para crear una orquestación que realiza lo siguiente:  
  
-   Actualice la columna PHOTO (del tipo de datos BLOB) de la tabla CUSTOMER.  
  
-   Leer el valor de la columna foto para el registro actualizado.  
  
 Esta orquestación está diseñada de tal manera que sólo proporcione el mensaje de solicitud para la operación de actualización en tiempo de ejecución. El mensaje para la operación de lectura se creará dentro de la operación.  
  
> [!NOTE]
>  La orquestación en este tema lee y actualiza los datos de la tabla CUSTOMER, que es una tabla de base de datos se crean mediante la ejecución de las secuencias de comandos que se proporciona con los ejemplos. Debe realizar procesos similares tal como se describe en este tema para operaciones de lectura u operaciones update en cualquier vista de tabla o la interfaz de la interfaz.  
  
## <a name="generating-schema"></a>Generar el esquema  
 Este tema muestra cómo realizar la lectura básico y operaciones update en una columna de fotografías (del tipo de datos BLOB) en una tabla de clientes. Esta tabla se crea mediante la ejecución de las secuencias de comandos que se proporciona con los ejemplos.  
...
## <a name="defining-messages-and-message-types"></a>Definición de mensajes y tipos de mensaje  
 ...
 En esta orquestación debe crear cuatro mensajes: establece una respuesta de recepción para el **Update_PHOTO** operación y la otra recepción-respuesta establecen para la **Read_PHOTO** operación.  
...
5.  En el **propiedades** panel para la **Message_1**, realice lo siguiente:  
    ...
    |Tipo de mensaje|En la lista desplegable, expanda **esquemas**y, a continuación, seleccione *LOBOperations.OracleEBSBinding.Update_PHOTO*, donde LOBOperations es el nombre de su proyecto de BizTalk. OracleEBSBindingSchema es el esquema generado para invocar la **Update_PHOTO** operación en la tabla CUSTOMER.|  
...
## <a name="setting-up-the-orchestration"></a>Configuración de la orquestación  
 En esta orquestación, el adaptador recibe un mensaje de solicitud para realizar una operación de Update_PHOTO en una tabla de clientes. El mensaje de notificación se recibe en una ubicación de archivo. El adaptador utiliza este mensaje y pasa a la base de datos de Oracle. La respuesta de base de datos de Oracle se guarda en otra ubicación. Una vez que se recibe la respuesta, la orquestación construye un mensaje para invocar la operación Read_PHOTO, que lee el valor de la columna PHOTO actualizado por la operación de Update_PHOTO. También se recibe la respuesta para este mensaje en la misma ubicación de archivo.  
  
 Por lo tanto, la orquestación debe contener lo siguiente:  
  
-   Puerto para quitar un mensaje de solicitud de recepción de un archivo **Update_PHOTO** operación.  
  
-   Un bidireccional de WCF-Custom o WCF-OracleEBS puerto de envío para enviar mensajes a ejecutar la **Update_PHOTO** operación.  
  
-   Un bidireccional de WCF-Custom o WCF-OracleEBS puerto de envío para enviar mensajes a ejecutar la **Read_PHOTO** operación. También puede realizar tanto **Read_PHOTO** y **Update_PHOTO** con el mismo WCF-Custom o WCF-OracleEBS puerto de envío. En este tema, utilizará un puerto de envío solo para las operaciones de ambos.  
  
-   A **construir mensaje** forma para construir mensajes dentro de la orquestación.  
...
|-----------|----------------|----------------|  
|ReceiveUpdateMessage|Receive|-Establecer **nombre** a *ReceiveUpdateMessage*<br />-Establecer **activar** a *es True*|  
...
|LOBPort|-Establecer **identificador** a *LOBPort*<br />-Establecer **tipo** a *LOBPortType*<br />-Establecer **patrón de comunicación** a *solicitudes y respuestas*<br />-Establecer **dirección de comunicación** a *envío y recepción*<br />-Crear una operación *Read_LOB*. Esta operación se usa para los mensajes para leer los valores de las columnas de tipo de datos de gran tamaño.<br />-Crear una operación *Update_LOB*. Esta operación se usa para los mensajes para actualizar valores en columnas de tipo de datos de gran tamaño.|  
|ResponseOut|-Establecer **identificador** a *ResponseOut*<br />-Establecer **tipo** a *ResponseOutType*<br />-Establecer **patrón de comunicación** a *unidireccional*<br />-Establecer **dirección de comunicación** a *enviar*<br />-Crear una operación *Read_LOB*. Esta operación se usa para los mensajes para leer los valores de las columnas de tipo de datos de gran tamaño.<br />-Crear una operación *Update_LOB*. Esta operación se usa para los mensajes para actualizar valores en columnas de tipo de datos de gran tamaño.|  
...
-   Asignación de los puertos que ha creado en la orquestación con puertos físicos en la [!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)] consola de administración. Para que esta orquestación debe:  
    ...
    -   Definir un puerto de envío WCF-Custom o WCF-OracleEBS físico para enviar mensajes a la base de datos de Oracle. También debe especificar la acción en el puerto de envío. Para obtener información sobre cómo crear puertos, consulte [configurar manualmente un puerto físico enlazado con el adaptador de Oracle E-Business](../../adapters-and-accelerators/adapter-oracle-ebs/manually-configure-a-physical-port-binding-to-the-oracle-e-business-adapter.md). Debe realizar las siguientes consideraciones al configurar el WCF-Custom o WCF-OracleEBS puerto de envío.  
        ...
        -   Dado el WCF-Custom o WCF-OracleEBS puertos de envío envían y recibe mensajes sean conformes a más de un esquema y realiza dos operaciones, debe establecer acción dinámico para las operaciones de ambos. Para obtener más información acerca de las acciones, vea [configurar la acción SOAP para Oracle E-Business Suite](../../adapters-and-accelerators/adapter-oracle-ebs/configure-the-soap-action-for-oracle-e-business-suite.md). Para esta orquestación, debe establecerse la acción como se indica a continuación:  
            ...
            > [!IMPORTANT]
            >  Tenga en cuenta que el nombre de la operación en una acción dinámico debe ser igual que el nombre de la operación especificada en los puertos lógicos durante la creación de la orquestación de BizTalk.  
...
## <a name="starting-the-application"></a>A partir de la aplicación  
 Antes de iniciar la orquestación de BizTalk, asegúrese de que la solicitud XML para invocar la **Read_PHOTO** operación está disponible en C:\TestLocation\MessageIn. La solicitud XML debe ser similar a lo siguiente:  
...
> [!NOTE]
>  El mensaje de solicitud tiene un filtro en el nombre específico, ya que en el mensaje de solicitud para **Update_PHOTO** operación, el valor de la columna PHOTO se actualizó con el mismo nombre. Por lo tanto, la operación de lectura leerá el mismo valor que se insertan manualmente mediante la operación de actualización.  
...
## <a name="executing-the-operation"></a>Ejecutar la operación  
 Después de iniciar la aplicación que se debe quitar un mensaje de solicitud en el archivo de ubicación de recepción. El esquema del mensaje de solicitud debe ajustarse al esquema para el **Update_PHOTO** operación generado anteriormente. Por ejemplo, un mensaje de solicitud que actualiza la columna PHOTO de la tabla CUSTOMER es similar a lo siguiente:  
...
> [!NOTE]
>  Al actualizar las columnas BLOB, el elemento de datos siempre debe contener un valor codificado en base64. Para obtener CLOB y NCLOB, el elemento de datos puede tener valores de cadena.  
  
 El mensaje de solicitud anterior actualiza el valor de la columna PHOTO para el registro que coincida con la cláusula WHERE. Ver los esquemas de mensaje para las operaciones en tipos de datos grandes para obtener más información acerca del esquema de mensaje de solicitud para realizar operaciones en tipos de datos de gran tamaño mediante el [!INCLUDE[adapteroraclebusinessshort](../../includes/adapteroraclebusinessshort-md.md)].  
...
> [!NOTE]
>  Tenga en cuenta que la respuesta contiene el mismo valor para la columna de fotos que se pasó en el **Update_PHOTO** operación.  

...
ms.locfileid: "25967858"
---
# <a name="receive-oracle-e-business-suite-change-notifications-incrementally-using-biztalk-server"></a>Recibir notificaciones de cambios de Oracle E-Business Suite incrementalmente mediante BizTalk Server
> [!IMPORTANT]
>  Por brevedad, en este tema solo se describe cómo recibir las notificaciones de forma incremental. En escenarios empresariales, la orquestación lo ideal es que debe incluir la lógica para extraer el tipo de mensaje de notificación recibido y, a continuación, realizar las operaciones siguientes. En otras palabras, se debe crear la orquestación se describe en este tema sobre la orquestación se describe en [mensajes de notificación de proceso para completar tareas específicas en Oracle E-Business Suite](../../adapters-and-accelerators/adapter-oracle-ebs/process-notification-messages-to-complete-specific-tasks-in-oracle-ebs.md).  
  
 Este tema muestra cómo configurar el [!INCLUDE[adapteroraclebusinessshort](../../includes/adapteroraclebusinessshort-md.md)] para recibir mensajes de notificación de consulta incremental de Oracle. Para mostrar notificaciones incrementales, consideramos que una tabla, ACCOUNTACTIVITY, con una columna "Procesados". Cuando se inserta un nuevo registro en esta tabla, el valor de la columna "Procesados" se establece en ' n '. Puede configurar el adaptador para recibir notificaciones incrementales haciendo lo siguiente:  
...
## <a name="how-this-topic-demonstrates-receiving-notification-messages"></a>Cómo en este tema se muestra cómo recibir mensajes de notificación  
 En este tema, para demostrar cómo [!INCLUDE[adapteroraclebusinessshort](../../includes/adapteroraclebusinessshort-md.md)] permite recibir mensajes de notificación de cambio incremental de la base de datos de Oracle E-Business Suite, se va a configurar el adaptador para recibir notificaciones de cambios en la tabla ACCOUNTACTIVTY. Supongamos que la tabla ACCOUNTACTIVITY tiene las columnas "TID", "Cuenta" y "Procesados". Cada vez que se agrega un nuevo registro, el valor de la columna "Procesados" se establece en ' n '. Por lo tanto, para obtener notificaciones incrementales que tendrá que hacer lo siguiente como parte de la orquestación de BizTalk:  
...
## <a name="how-to-receive-notification-messages-from-oracle-e-business-suite"></a>Cómo recibir mensajes de notificación de Oracle E-Business Suite  
 Realizar una operación sobre el uso de Oracle E-Business Suite [!INCLUDE[adapteroraclebusinessshort](../../includes/adapteroraclebusinessshort-md.md)] con [!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)] conlleva las tareas de procedimientos descritas en [bloques de creación para crear aplicaciones de Oracle E-Business Suite](../../adapters-and-accelerators/adapter-oracle-ebs/building-blocks-to-create-oracle-e-business-suite-applications.md). Para configurar el adaptador para recibir mensajes de notificación, estas tareas son:  
...
4.  Crear una orquestación que realiza lo siguiente:  
    ...
    -   Envía este mensaje a Oracle E-Business Suite para seleccionar y actualizar los registros y recibir una respuesta.  
...
## <a name="defining-messages-and-message-types"></a>Definición de mensajes y tipos de mensaje  
 ...
 En este tema, debe crear tres mensajes: uno para recibir notificaciones de Oracle E-Business Suite, uno para ejecutar el procedimiento PROCESS_RECORDS y otro para recibir la respuesta para el procedimiento.  
...
|SaveProcResponse|-Establecer **mensaje** a *ProedureResponse*<br /><br /> -Establecer **operación** a *SaveMessagePort.Procedure.Request*|  
 ...
 Ahora debe compilar la solución de BizTalk e implementarlo en un [!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]. Para obtener más información, vea "Crear y ejecutar orquestaciones" en [http://go.microsoft.com/fwlink/?LinkId=102359](http://go.microsoft.com/fwlink/?LinkId=102359).  
...
-   Asignación de los puertos que ha creado en la orquestación con puertos físicos en la [!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)] consola de administración. Para que esta orquestación debe:  
    ...
    -   Definir una ubicación en el disco duro y un puerto de archivo correspondiente donde la orquestación de BizTalk colocará los mensajes de Oracle E-Business Suite. Estos serán los mensajes de notificación recibidos desde Oracle E-Business Suite y puerto de envío de mensajes para el procedimiento PROCESS_RECORDS que ejecutar a través de WCF-Custom o WCF-OracleEBS.  
...
-   Puerto de recepción de la WCF-Custom o WCF-OracleEBS unidireccional que recibe los mensajes de notificación de Oracle E-Business Suite está ejecutando.  
  
-   Se está ejecutando el WCF-Custom o el puerto de envío WCF-OracleEBS para ejecutar el procedimiento PROCESS_RECORDS.  
...
-   El adaptador ejecuta el procedimiento PROCESS_RECORDS. La siguiente respuesta de Oracle E-Business Suite es para el procedimiento.  

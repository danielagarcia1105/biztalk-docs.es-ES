...
Para implementar un adaptador, debe instalar al ensamblado del adaptador en la caché de ensamblados global (GAC) y, a continuación, registrar el adaptador en el archivo machine.config.  
  
## <a name="install-the-adapter-assembly-into-the-gac"></a>Instale al ensamblado del adaptador en la GAC  
...
7. En el **nombre seguro** cuadro, haga clic en el campo junto a **archivo clave de ensamblado**, haga clic en el botón Examinar (**...** ) y, a continuación, busque el archivo de clave.  
  
8. Haga clic en el archivo de clave, haga clic en **abierto**y, a continuación, haga clic en **Aceptar**.  
  
9. En el menú de Visual Studio, haga clic en **compilar**y, a continuación, elija **compilar solución**.  
  
> [!NOTE]
>  Si el ensamblado del adaptador depende de otros ensamblados, asegúrese de que se firman los ensamblados de referencia con un nombre seguro; en caso contrario, obtendrá un error.  
  
 Si tiene el origen, puede volver a compilar con un nombre seguro como se mostró anteriormente. Si el ensamblado de referencia pertenece a un tercero y no está seguro de que se le asignará un nombre seguro, puede desensamblar y, a continuación, volver a ensamblar el ensamblado con un nombre seguro.  
  
 El ensamblado original se sobrescribirá, por tanto, si desea conservar la versión original, asegúrese realice una copia de seguridad del mismo antes de continuar con los pasos siguientes.  
  
 Use el Desensamblador de lenguaje intermedio de Microsoft (MSIL) para desensamblar el ensamblado:  
  
- ILDASM thirdparty.dll /out:thirdparty.il  
  
  Use el ensamblador de MSIL para volver a ensamblar el ensamblado con un nombre seguro:  
  
- ILASM thirdparty.il /dll /key=strongkeyfile.snk  
  
#### <a name="install-an-assembly-in-the-gac"></a>Instalar a un ensamblado en la GAC  
  
1. Compruebe que el adaptador se ha firmado.  
...
## <a name="register-the-adapter-in-machineconfig"></a>Registrar el adaptador en el archivo Machine.config  
 Un adaptador desarrolladas mediante el [!INCLUDE[afproductnameshort](../../includes/afproductnameshort-md.md)] aparece como un enlace WCF.  Para obtener más información, consulte Microsoft.ServiceModel.Channels.Common.AdapterBinding.  El enlace del adaptador se registra con WCF mediante \<bindingExtensions\> sección \<sistema. ServiceModel\> y el elemento de enlace de transporte de adaptador está registrado con WCF mediante \<bindingElementExtensions\> sección \<sistema. ServiceModel\>.  
...
2. Actualice el archivo machine.config. Si el archivo no contiene una sección de system.serviceModel, agregue la siguiente sección al final del archivo de configuración, pero antes de que el cierre de raíz de etiqueta.  
...
3. Cierre y guarde el archivo machine.config.  
   ...
   También puede usar el [Editor de configuración del servicio](https://msdn.microsoft.com/library/ms732009.aspx) para modificar el archivo machine.config.
  
#### <a name="edit-the-machineconfig-file-using-the-service-configuration-editor"></a>Edite el archivo machine.config mediante el Editor de configuración de servicio  
...
5.  Haga clic en el icono de global de ensamblados (GAC) de la caché para obtener una lista de los archivos DLL en la GAC.  

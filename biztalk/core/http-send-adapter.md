---
title: Adaptador de envío HTTP | Microsoft Docs
ms.custom: ''
ms.date: 06/08/2017
...
## <a name="chunked-encoding-support-for-the-http-send-adapter"></a>Compatibilidad de codificación fragmentada del adaptador de envío HTTP  
 Si el **Habilitar codificación fragmentada** está habilitada la opción de configuración y, después, el adaptador de envío HTTP envía mensajes de solicitud mediante la codificación fragmentada si el tamaño de la solicitud es superior a 8 KB. Cuando se utiliza el servidor proxy de HTTP, el adaptador de envío HTTP no utiliza la codificación fragmentada, y siempre almacena los datos antes de enviarlos. El **Habilitar codificación fragmentada** predeterminada está habilitada la opción de configuración.  
...
- **Básico.** El adaptador de HTTP envía el nombre de usuario y la contraseña a través de una conexión HTTP de texto sin formato.  
...
-   Debe especificar la misma cuenta de usuario en los siguientes lugares:  
    ...
    -   La identidad del grupo de aplicaciones (IIS 7.0) o identidad de aplicaciones COM+ de host del directorio IIS virtual al que supervisa el adaptador de recepción HTTP. Para obtener más información acerca de cómo configurar IIS para HTTP las ubicaciones de recepción, vea [cómo configurar IIS para una ubicación de recepción HTTP](../core/how-to-configure-iis-for-an-http-receive-location.md).  
  
    -   Las credenciales de inicio de sesión utilizadas para la instancia de host aislado que se ejecuta el adaptador HTTP en. Para obtener información sobre cómo configurar las credenciales de inicio de sesión para una instancia de host, consulte [cómo modificar propiedades de instancia de Host](../core/how-to-modify-host-instance-properties.md).  
  
-   El host aislado que esté utilizando el adaptador de HTTP deberá configurarse con la opción Autenticación de confianza. Para obtener información acerca de cómo configurar un host como autenticación de confianza, consulte [cómo modificar las propiedades de Host](../core/how-to-modify-host-properties.md).  
...
- Crear un puerto de envío con un filtro para la propiedad de contexto de mensaje en cuestión. Consulte **las propiedades de contexto de mensaje** [!INCLUDE[ui-guidance-developers-reference](../includes/ui-guidance-developers-reference.md)] para obtener una lista de propiedades de contexto de mensaje del sistema, incluidas aquéllas relativas a la confirmación de mensaje.  
...
**Objetos COM de inicio de sesión único** [!INCLUDE[ui-guidance-developers-reference](../includes/ui-guidance-developers-reference.md)]
...
5.  El adaptador llama **realiza** en el lote, pasa su transacción MSDTC y su **BatchComplete** puntero de devolución de llamada. El motor devuelve una **IBTDTCCommitConfirm** interfaz.  
  
6.  El motor procesa el lote, se llama de nuevo el adaptador en su **BatchComplete** implementación y transmite el estado de su mensaje de procesamiento para el adaptador.  
  
7.  Si el lote se realizó correctamente el adaptador confirma la transacción y llama el **IBTDTCCommitConfirm.DTCCommitConfirm** API con una `true` valor que significa confirmar.  
 ...
 **Sugerencia para la implementación:** para envíos de transacción, el adaptador debe usar la misma transacción MSDTC para escribir los datos en el destino y para eliminar a través de la **IBTTransportBatch.DeleteMessage** llamada al método. solo requieren transacciones estas dos operaciones. Todas las demás operaciones, como **IBTTransportBatch.Resubmit**, **IBTTransportBatch.MoveToNextTransport**, y **IBTTransportBatch.MoveToSuspendQ** no es necesario transacción. Esto se debe a que el motor usa de manera implícita una transacción, y estos tipos de operaciones no necesitan ser atómicas con respecto a su destino.  
...
3.  El motor llama **realiza** en el lote, haciendo que el adaptador enviar el lote a su cola de transmisión interna que se repara por su grupo de subprocesos.  
  
4.  El adaptador crea una nueva transacción MSDTC.  
  
5.  El adaptador transmite los mensajes dando de alta el destino en la transacción MSDTC. Por ejemplo, podría hacerlo escribiendo en un [!INCLUDE[btsSQLServerNoVersion](../includes/btssqlservernoversion-md.md)] base de datos.  
  
6.  Después de la transmisión, el adaptador obtiene un nuevo lote del motor.  
  
7.  El adaptador llama **DeleteMessage** para los mensajes que ha transmitido correctamente.  
  
8.  El adaptador llama **realiza** en el lote y pasa su transacción DTC. El motor devuelve una **IBTDTCCommitConfirm** interfaz.  
  
9. El motor procesa el lote y elimina los mensajes de la cola de la aplicación.  
  
10. El motor llama al adaptador **IBTBatchCallback** interfaz con información sobre el éxito de sus operaciones de eliminación.  
  
11. Si el lote se ha procesado correctamente, el adaptador confirma las transacciones.  
  
12. El adaptador llama **IBTDTCCommitConfirm.DTCCommitConfirm** para informar al motor que la transacción se ha confirmado correctamente.  
...
### <a name="transacted-solicit-response-adapters"></a>Adaptadores con transacciones de petición-respuesta  
 Al contrario que las recepciones bidireccionales, los envíos bidireccionales se pueden realizar mediante la misma transacción DTC. Adaptadores de transacción petición-respuesta deben usar la misma **IBTTransportBatch** para el **SubmitResponseMessage** y **DeleteMessage** operaciones. Este lote debe usar la misma transacción MSDTC usada para enviar y recibir el par de mensajes de petición-respuesta. De este modo se garantiza la atomicidad para el intercambio de mensajes de petición-respuesta.  
...
-   **DTCConfirmCommit** es un método en el **IBTDTCCommitConfirm** objeto. Una instancia de la **IBTDTCCommitConfirm** objeto devuelto por el lote **ibttransportbatch:: Done** llamar. Esta instancia se encuentra en el mismo subproceso que el **ibttransportbatch:: Done** llamar a, que es diferente del subproceso del adaptador.  
  
-   Para cada llamada que el adaptador efectúa a **Ibttransportbatch** hay una devolución de llamada correspondiente, **BatchComplete**, que es llamado por el motor de mensajería en un subproceso independiente para informar del resultado de el envío del lote. En **BatchComplete** el adaptador debe confirmar o revertir la transacción en función de si el lote superado o no superado. En cualquier caso, el adaptador debe llamar **DTCConfirmCommit** para informar del estado de la transacción al motor de mensajería.  
...
```  
 ...
 Ahora asigne el valor devuelto de **Ibttransportbatch** a esta propiedad y usarla en el **BatchComplete** llamar a.  

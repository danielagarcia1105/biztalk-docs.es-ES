---
title: Administrar definiciones de BAM | Documentos de Microsoft
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- managing [BAM definitions]
- definitions [BAM], managing
- definition files [BAM], managing
- managing [BAM], definitions
- managing [BAM definitions], about managing BAM definitions
ms.assetid: 7aba0e40-b8d3-4afc-9e4c-92392f1f6269
caps.latest.revision: "10"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 93cf3d280d0e615442a4141b7fa41f6ee3566490
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 09/20/2017
---
# <a name="managing-bam-definitions"></a>Administrar definiciones de BAM
Una definición de BAM es una parte de la infraestructura de BAM. Establece los datos que se van a agregar y de los que se va a realizar el seguimiento, así como la vista del usuario empresarial final en los datos de seguimiento. Los temas de esta sección explican los procedimientos para administrar los elementos de las definiciones de BAM, que incluyen actividades, vistas, artefactos y alertas.  
  
## <a name="in-this-section"></a>En esta sección  
  
-   [Derechos de usuario necesarios para administrar archivos de definición de BAM](../core/required-user-rights-for-managing-bam-definition-files.md)  
  
-   [Cómo implementar definiciones de BAM](../core/how-to-deploy-bam-definitions.md)  
  
-   [Cómo quitar definiciones de BAM](../core/how-to-remove-bam-activities.md)  
  
-   [Cómo enumerar los cambios en la infraestructura de BAM](../core/how-to-list-changes-to-the-bam-infrastructure.md)  
  
-   [Cómo enumerar actividades de BAM](../core/how-to-list-bam-activities.md)  
  
-   [Cómo quitar actividades BAM](../core/how-to-remove-bam-activities.md)  
  
-   [Cómo enumerar vistas de BAM](../core/how-to-list-bam-views.md)  
  
-   [Cómo quitar vistas de BAM](../core/how-to-remove-bam-views.md)  
  
-   [Cómo habilitar las alertas](../core/how-to-enable-alerts.md)  
  
-   [Cómo deshabilitar las alertas](../core/how-to-disable-alerts.md)  
  
-   [Cómo quitar alertas BAM](../core/how-to-remove-bam-alerts.md)  
  
-   [Cómo actualizar artefactos de BAM](../core/how-to-update-bam-artifacts.md)  
  
-   [Cómo quitar artefactos implementados](../core/how-to-remove-deployed-artifacts.md)  
  
-   [La configuración de alertas BAM](../core/configuring-bam-alerts.md)  
  
-   [Administrar la ejecución de alertas de BAM](../core/managing-bam-alert-execution.md)  
  
-   [Cómo cambiar la frecuencia con las alertas que se evalúan](../core/how-to-change-the-frequency-with-which-alerts-are-evaluated.md)
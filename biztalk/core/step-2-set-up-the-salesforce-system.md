...
ms.locfileid: "22279932"
---
# <a name="step-2-set-up-the-salesforce-system"></a>Paso 2: Configurar el sistema Salesforce
...
2.  En el portal, haga clic en el **cuentas** ficha y, a continuación, haga clic en **nuevo**.  
  
3.  En el **nueva cuenta** , proporcione valores para los distintos campos. Especificar un valor para **nombre de la cuenta** es obligatorio. Para este tutorial, especifique el nombre de cuenta como `Customer1`.  
...
2.  En el **oportunidades recientes** sección, haga clic en **nuevo**.  
...
3.  En la página Oportunidad nueva, especifique los siguientes valores:  
    ...
    2.  Especifique el **nombre de cuenta**. Este nombre representa la cuenta con la que está asociada esta oportunidad. Para este tutorial, establezca la cuenta `Customer1`. Esta cuenta se crea en el procedimiento anterior.  
  
    3.  Especifique un **fecha de cierre**. Esta es la fecha en la que debería cerrarse la oportunidad.  
  
    4.  Especifique un **fase**. Este valor denota la etapa actual para la oportunidad. Comenzar con, puede establecer la oportunidad en cualquier valor, por ejemplo, **necesita análisis**.  
  
         ![Crear una oportunidad en Salesforce](../core/media/bts-sf-create-opp.jpg "BTS_SF_Create_Opp")  
  
        > [!NOTE]
        >  Asegúrese de que no establece la etapa en **Closed Won** comenzar con. Para el escenario de este tutorial, cada vez que se establece la etapa **Closed Won** se envía una notificación a un extremo de retransmisión [!INCLUDE[winazure](../includes/winazure-md.md)] [!INCLUDE[sb](../includes/sb-md.md)]. No hemos configurado esa parte de la solución, por lo que no debe preparar el terreno **Closed Won**.  
  
    5.  Especificar valores para otros campos opcionales y, a continuación, haga clic en **guardar**.  
  
    6.  En la página oportunidad para Customer1, en la **productos** sección, haga clic en **agregar producto**.  
  
    7.  En la lista de productos, seleccione los productos que el cliente está interesado en y, a continuación, haga clic en **seleccione**.  
  
    8.  Para cada producto seleccionado, especifique una cantidad que el cliente desee y, a continuación, haga clic en **guardar**.  
...
## <a name="create-a-salesforce-workflow"></a>Crear un flujo de trabajo en Salesforce  
 En este paso, crearemos un flujo de trabajo para enviar una notificación cada vez que se cierre una oportunidad satisfactoriamente. La notificación tiene el formato de un mensaje SOAP y se envía a un extremo de retransmisión hospedado en [!INCLUDE[winazure](../includes/winazure-md.md)] [!INCLUDE[sb](../includes/sb-md.md)].  
  
#### <a name="to-create-a-workflow-for-opportunities"></a>Para crear un flujo de trabajo para oportunidades  
  
1.  En el portal de Salesforce, haga clic en el nombre de inicio de sesión en la esquina superior derecha de la página y, a continuación, haga clic en **el programa de instalación**.  
...
4.  Desde el **seleccionar un objeto de** lista, haga clic en **oportunidad**y, a continuación, haga clic en **siguiente**.  
  
5.  En la página siguiente, especifique estos datos:  
  
    1.  Establecer el **nombre de regla** como `Closed Opportunity`.  
  
    2.  Establecer el **criterios de evaluación** como **creado y cada vez que es modificado para cumplir los criterios**.  
  
    3.  Para el **criterios de la regla**, establezca ejecutar la regla cuando el **se cumplen los criterios**.  
...
6.  Defina la acción del flujo de trabajo para la regla nueva:  
    ...
    4.  Especifique el **dirección URL del extremo** como `https://btssalesforce.servicebus.windows.net/notifications/opportunity`.  
         ...
         En este caso, **btssalesforce** es su [!INCLUDE[sb](../includes/sb-md.md)] espacio de nombres que ha creado en los pasos anteriores. **/Notifications/opportunity/** representa la retransmisión que creamos en pasos posteriores de este tutorial.  
    ...
    8.  En el panel izquierdo, bajo **el programa de instalación de la aplicación**, expanda **crear**, expanda **flujo de trabajo y aprobaciones**y, a continuación, haga clic en **reglas de flujo de trabajo**. Compruebe que la **oportunidad cerrada** regla está en la lista. En el **acción** columna para la **oportunidad cerrada** de regla, haga clic en **activar** para activar la regla.  
...
2.  En el panel izquierdo, bajo **el programa de instalación de la aplicación**, expanda **crear**y, a continuación, expanda **aplicaciones**. En el **aplicaciones** página, en la **aplicaciones conectadas** sección, haga clic en **nuevo**.  
  
3.  En el **nueva aplicación conectada** página, especifique lo siguiente:  
  
    1.  Para **nombre de aplicación conectada**, especifique `BizTalk_Salesforce`.  
  
    2.  Para **nombre del desarrollador**, especifique el registro en nombre.  
    ...
    5.  En **ámbitos de OAuth disponibles**, seleccione **acceso completo**, **realizar solicitudes en su nombre en cualquier momento**, y **acceso y administrar datos**y, a continuación, haga clic en el **agregar** botón para moverlos a la **seleccionado ámbitos de OAuth**.  
    ...
    6.  Haga clic en **Guardar**. La página que aparece contiene información sobre la **clave de consumidor** y **secreto de consumidor**. Debe tomar nota de estos valores. Los necesitará para conectarse a Salesforce desde [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)].  
         ...
         ![Teclas de acceso de Salesforce](../core/media/bts-sf-consumer-keys.jpg "BTS_SF_Consumer_Keys")  

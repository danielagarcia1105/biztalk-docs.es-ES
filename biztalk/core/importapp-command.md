...
Importa los artefactos contenidos en un archivo .msi a una aplicación de BizTalk. Si la aplicación no existe, se creará.  
 ...
 Al importar una aplicación, puede utilizar el parámetro /Environment para especificar el entorno de desarrollo de destino para la aplicación, si ha agregado archivos de enlace a esta aplicación que se personaliza para un entorno de desarrollo determinado. Para obtener información general, consulte [archivos de enlace e implementación de aplicaciones](../core/binding-files-and-application-deployment.md). Para obtener instrucciones sobre cómo agregar archivos de enlace, consulte [AddResource (comando): enlace de BizTalk](../core/addresource-command-biztalk-binding.md).  
...
|**/ ApplicationName** (o **/A**, vea la sección Comentarios)|no|Nombre de la aplicación de BizTalk a la que se importan los artefactos del archivo .msi. Si el nombre incluye espacios, debe encerrar entre comillas dobles ("). Si no se especifica, se utiliza la aplicación predeterminada. Si la aplicación especificada no existe, se creará la aplicación.|  

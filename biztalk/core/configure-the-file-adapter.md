...
Puede crear una ubicación de recepción y envío puerto mediante [!INCLUDE[btsBizTalkServerAdminConsoleui](../includes/btsbiztalkserveradminconsoleui-md.md)], o mediante programación. En este tema se centra en la [!INCLUDE[btsBizTalkServerAdminConsoleui](../includes/btsbiztalkserveradminconsoleui-md.md)] consola. Para conocer los pasos mediante programación, vaya a [crear la ubicación de recepción o puerto de envío mediante programación](../core/create-the-receive-location-and-send-port-programmatically.md).

> [!IMPORTANT]
> **A partir de [!INCLUDE[bts2016_md](../includes/bts2016-md.md)]** , puede conectarse a un recurso compartido de archivos de Azure mediante el adaptador de archivo. La cuenta de almacenamiento de Azure se montará en el servidor BizTalk Server. [Introducción a Azure File storage en Windows](https://docs.microsoft.com/azure/storage/storage-dotnet-how-to-use-files#mount-the-file-share) se enumeran los pasos de montaje.
...
Controladores de adaptador que se ejecutan en el contexto de seguridad de la instancia de host seleccionada para el controlador de adaptador. La instancia de host usa la `Logon` propiedad en el  ***nombre de Host* -propiedades de la instancia de Host** en administración de BizTalk. Esto `Logon` cuenta debe tener permisos específicos para las carpetas o recursos compartidos utilizados por el adaptador de archivo.
...
| --- | --- | --- |
| Control total | ✔ <br/> en el nivel compartido (si el acceso a un recurso compartido de archivos) |  | 
...
| Cambiar | | ✔ <br/> en el nivel compartido (si el acceso a un recurso compartido de archivos) |
...
3. En el panel izquierdo de la **propiedades de puerto de recepción** cuadro de diálogo, seleccione **ubicaciones de recepción**y en el panel derecho haga doble clic existente de ubicación de recepción o haga clic en **New** a Cree una nueva ubicación de recepción.  

4. En el **propiedades de ubicación de recepción** cuadro de diálogo el **transporte** sección, seleccione **archivo** tipo en la lista desplegable y, a continuación, haga clic en **configurar**  para configurar las propiedades de transporte para la ubicación de recepción.  
...
5. En el **General** ficha, realice lo siguiente:  
   ...
   |         Use         |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      Para                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
   |--------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
   |    **Carpeta de recepción**    | Requerido. Escriba la ruta de acceso a una carpeta en el sistema de archivos, el recurso compartido de red, o el recurso compartido de archivos de Azure que el archivo de controlador de recepción lee archivos. Puede especificar la ruta de acceso directamente en el **carpeta recepción** texto cuadro o selecciónelo en el sistema de archivos mediante el **examinar** botón. Al buscar la carpeta, también puede crear una nueva carpeta mediante **crear nueva carpeta**.<br /><br /> Si usa un recurso compartido de archivos de Azure storage, escriba `\\yourfilestoragename.file.core.windows.net\yourfilesharename`. <br /><br />**Tipo:** cadena <br/><br/>**Nota:** no establezca la **carpeta recepción** propiedad en una carpeta que usa el sistema de archivos distribuido de NT con un vínculo simbólico. Si utiliza un sistema de archivos distribuido de NT, solo puede usar carpetas con rutas de acceso de red directas en el adaptador de archivo de las ubicaciones de recepción. <br /><br /> Para las restricciones en esta propiedad, vea [restricciones al configurar el adaptador de archivo](../core/restrictions-when-configuring-the-file-adapter.md). <br/><br/>**Nota:** el URI para un envío de puerto o recibir ubicación no puede superar los 256 caracteres. |
   ...
   | **Intervalo (min) de reintentos** |                                                                                                                                                                                                                                                                                                                                                                                           Especificar el tiempo de intervalo de reintentos (en minutos) entre los intentos de obtención de acceso a la ubicación de recepción del recurso compartido de red que no está disponible temporalmente.<br /><br /> **Valor predeterminado:** 5 minutos<br /><br /> **Tipo:** largo<br /><br /> **Valor mínimo:** 0<br /><br /> **Valor máximo:** MAX_LONG                                                                                                                                                                                                                                                                                                                                                                                            |
...
6. En el **autenticación** ficha, realice lo siguiente:  
   ...
   |                                 Use                                  |                                                                                                                                                                                                                                                                                   Para                                                                                                                                                                                                                                                                                    |
   |---------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
   | **Use estas credenciales cuando el host no tenga acceso al recurso compartido de red** |                                                                                                                                                                                                                      Solo es necesario si usa un recurso compartido de red o un recurso compartido de archivos de Azure. <br /><br /> **Valor predeterminado:** False<br /><br /> **Tipo:** booleano                                                                                                                                                                                                                      |
   |                               **Nombre de usuario.**                               | Si usa un recurso compartido de red, escriba el nombre de usuario que tiene acceso al recurso compartido. <br /><br />  Si el recurso compartido mediante un almacenamiento de archivos de Azure, escriba el nombre de la cuenta de almacenamiento.<br/><br/>**Tipo:** cadena <br /><br />**Nota:** si varias ubicaciones de recepción asignadas al mismo recurso compartido de red se configuran con credenciales alternativas, a continuación, se deben usar las mismas credenciales para todas las ubicaciones de recepción. Windows no permite establecer varias conexiones a un servidor de red compartido desde el mismo equipo si se intenta usar más de un conjunto de credenciales. |
   |                               **Contraseña**                                |                                                                                                                                                          Si usa un recurso compartido de red, escriba la contraseña de la cuenta que tenga acceso al recurso compartido de red.<br /><br />  Si el recurso compartido mediante un almacenamiento de archivos de Azure, escriba la clave de acceso de cuenta de almacenamiento; que aparece en el portal de Azure.<br /><br /> **Tipo:** cadena                                                                                                                                                          |
...
9. Escriba los valores apropiados en el **propiedades de ubicación de recepción** cuadro de diálogo para completar la configuración de la ubicación de recepción y haga clic en **Aceptar** para guardar la configuración. Para obtener información sobre la **propiedades de las ubicaciones de recepción** cuadro de diálogo, vea [cómo crear una ubicación de recepción](../core/how-to-create-a-receive-location.md).  

## <a name="configure-the-send-port"></a>Configurar el puerto de envío

1.  En la consola de administración de BizTalk Server, cree un nuevo puerto de envío o haga doble clic en un puerto de envío existente para modificarlo. Consulte [cómo crear un puerto de envío](../core/how-to-create-a-send-port2.md). Configure todas las opciones de puerto de envío y especifique **archivo** para el **tipo** opción el **transporte** sección de la **General** ficha.  
...
4.  En el **autenticación** ficha, realice lo siguiente:  
    ...
    |--------------|----------------|  
    |**Use estas credenciales cuando el host no tenga acceso al recurso compartido de red**| Solo es necesario si usa un recurso compartido de red o un recurso compartido de archivos de Azure. <br /><br /> **Valor predeterminado:** False<br /><br /> **Tipo:** booleano|  
    |**Nombre de usuario.**|Si usa un recurso compartido de red, escriba el nombre de usuario que tiene acceso al recurso compartido. <br /><br />  Si el recurso compartido mediante un almacenamiento de archivos de Azure, escriba el nombre de la cuenta de almacenamiento.<br /><br /> **Tipo:** cadena|  
    |**Contraseña**|Si usa un recurso compartido de red, escriba la contraseña de la cuenta que tenga acceso al recurso compartido de red.<br /><br />  Si el recurso compartido mediante un almacenamiento de archivos de Azure, escriba la clave de acceso de cuenta de almacenamiento; que aparece en el portal de Azure.<br /><br /> **Tipo:** cadena|  
...
## <a name="set-the-properties-for-a-dynamic-send-port"></a>Establecer las propiedades de puerto de envío dinámico
 ...
 Un puerto de envío dinámico no proporciona ninguna opción de configuración de transporte en la consola de administración de BizTalk Server porque se espera que estas propiedades se proporcionen con las propiedades de contexto asociadas al mensaje. Estas propiedades se pueden establecer en una canalización personalizada o en una orquestación. Para establecer las propiedades de configuración en una orquestación, puede hacer lo siguiente:  
...
-   Agregue código a la **asignación de mensajes** forma para inicializar el mensaje construido y establecer las propiedades de configuración adecuado para el mensaje. El siguiente código inicializa un mensaje que se llama message_2 y que se construyó con un **construir mensaje** dar forma y, a continuación, establece dos propiedades de configuración para el mensaje. En este escenario, el Message_1 lo recibió originalmente la orquestación:  

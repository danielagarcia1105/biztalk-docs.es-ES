---
title: Administrar agregaciones | Documentos de Microsoft
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- managing [BAM], aggregations
- aggregations [BAM], managing
ms.assetid: 54b11cec-1bc4-4c40-a8f8-9f3a40f3161f
caps.latest.revision: "6"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: f1d645db5b109383ace2a7870dbb01000689b1d1
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 09/20/2017
---
# <a name="managing-aggregations"></a>Administrar agregaciones
Las agregaciones son operaciones acumulativas de un conjunto de datos que derivan en una característica generalmente representativa o descripción del conjunto de datos. En esta sección se describe cómo administrar elementos distintos de una agregación, como índices, actividades y agregaciones en tiempo real (ATR).  
  
 Para obtener información general acerca de las agregaciones, consulte [agregaciones en el Portal de BAM](../core/aggregations-in-the-bam-portal.md) y [agregaciones en la página de Portal de BAM](../core/aggregations-on-the-bam-portal-page.md).  
  
## <a name="in-this-section"></a>En esta sección  
  
-   [Cómo crear un índice](../core/how-to-create-an-index.md)  
  
-   [Cómo obtener una lista de índices en una agregación](../core/how-to-get-a-list-of-indexes-on-an-aggregation.md)  
  
-   [Cómo eliminar un índice](../core/how-to-delete-an-index.md)  
  
-   [Cómo establecer la duración de una ventana actividad](../core/how-to-set-the-duration-on-an-activity-window.md)  
  
-   [Cómo obtener la duración en una ventana actividad](../core/how-to-get-the-duration-on-an-activity-window.md)  
  
-   [Cómo establecer la duración de una ventana ATR](../core/how-to-set-the-duration-on-an-rta-window.md)  
  
-   [Cómo obtener la duración en una ventana ATR](../core/how-to-get-the-duration-on-an-rta-window.md)  
  
-   [Cómo definir agregaciones de BAM](../core/how-to-define-bam-aggregations.md)
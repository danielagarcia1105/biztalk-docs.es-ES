---
title: (Ejemplo de BizTalk Server) del adaptador de archivo | Microsoft Docs
ms.custom: ''
ms.date: 06/08/2017
...
>  El ejemplo de adaptador de archivo no es el mismo que el adaptador de archivo nativo que se incluye con BizTalk Server. Por lo tanto, al seleccionar el tipo de transporte que se va a utilizar con este ejemplo seleccione “estático” en lugar de Archivo.  
 ...
 El adaptador dinámico, al tener una interfaz de usuario personalizada y posiblemente un conjunto de esquemas dinámico, requerirá código adicional en la administración del adaptador. Para entender mejor el uso del conjunto de esquemas dinámico, vea [configuración del adaptador de tiempo de diseño dinámico](../core/dynamic-design-time-adapter-configuration.md).  
...
## <a name="where-to-find-this-sample"></a>Ubicación del ejemplo  
 \<*Ejemplos de la ruta de acceso*\>**\AdaptersDevelopment\File adaptador**  

> [!NOTE]
>  La ubicación predeterminada de \< *ruta de ejemplos* \> es *% ProgramFiles %* \Microsoft BizTalk Server\SDK\Samples cuando BizTalk Server se instala en un equipo de 32 bits versión de Windows. La ubicación predeterminada de \< *ruta de ejemplos* \> es *(x 86) %* \Microsoft BizTalk Server\SDK\Samples cuando BizTalk Server se instala en un equipo que ejecuta 64 - versión de bits de Windows. Para determinar los valores asociados con el *% ProgramFiles %* o *(x 86) %* tipo de variables de entorno **echo % ProgramFiles %** o **echo % (X 86) %** en un símbolo del sistema y presione ENTRAR. Si ejecuta este ejemplo en un sistema operativo de 64 bits, deberá cambiar todas las referencias en cualquiera de los archivos .reg de **% ProgramFiles %** a **(x 86) %** antes de ejecutar estos archivos.  
...
3.  Para volver a generar el proyecto de tiempo de ejecución de receptor del adaptador, en el Explorador de soluciones, haga clic en **DotNetFile**y, a continuación, haga clic en **recompilar**.  
...
1.  En el Explorador de Windows, navegue hasta la \< *ruta de ejemplos*\>**"\AdaptersDevelopment\File Adapter\Design Time\Adapter Management"** directory y, a continuación, haga doble clic en **Y AdapterManagement.sln**.  

2.  En el Explorador de soluciones, haga clic en **AdapterManagement**y, a continuación, haga clic en **recompilar**.  
...
2. Para agregar el adaptador de ejemplo en el registro, haga doble clic en **StaticAdapterManagement.reg**.  
   ...
   > [!NOTE]
   >  StaticAdapterManagement.reg incluye rutas codificadas de forma rígida en C:\Program Files\Microsoft BizTalk Server\\. Si no instaló [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] en el %ProgramFiles%\Microsoft BizTalk Server\ directorio, si ha actualizado la instalación de BizTalk Server de BizTalk Server 2009 o BizTalk Server 2006 R2, o si ha instalado BizTalk Server en un equipo que ejecuta un versión de 64 bits de Windows, debe modificar el archivo StaticAdapterManagement.reg con las rutas apropiadas. De forma predeterminada, BizTalk Server está instalado en el % ProgramFiles(x86) %\Microsoft BizTalk Server\ directorio en los equipos que ejecutan una versión de 64 bits de Windows. Actualice las rutas asociadas a los valores "InboundAssemblyPath", "OutboundAssemblyPath" y "AdapterMgmtAssemblyPath" para que señalen la ubicación correcta de los archivos especificados.  
   > 
   > [!IMPORTANT]
...
4. Para cerrar el Explorador de Windows, en el **archivo** menú, haga clic en **cerrar**.  
...
4. En el **agregar adaptador** diálogo cuadro, realice lo siguiente.  
   ...
   | **Comentario** |               Tipo **adaptador de ejemplo**.                |
...
2. En la unidad de instalación de BizTalk Server, cree las siguientes carpetas:  
...
3. Para cerrar el Explorador de Windows, en el **archivo** menú, haga clic en **cerrar**.  

4. Haga clic en **iniciar**, apunte a **todos los programas**, apunte a [!INCLUDE[btsBizTalkServerStartMenuItemui](../includes/btsbiztalkserverstartmenuitemui-md.md)]y, a continuación, haga clic en **administración de BizTalk Server**.  

5. Haga clic en el **administración de BizTalk Server** nodo y seleccione **conectar a grupo existente**.  

6. En el **conectarse a base de configuración de servidor BizTalk existente** diálogo cuadro, realice lo siguiente.  
   ...
   |    Use    |                                                                                    Para                                                                                     |
   |----------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
   | **SQL Server** |                                                                              Tipo **.** (un punto).                                                                               |
   |  **Base de datos**  | Seleccionar el nombre de la base de datos de administración de BizTalk que creó el Asistente para configuración. Es el nombre de base de datos predeterminado utilizado por el Asistente para configuración **BizTalkMgmtDb**. |
...
11. En el **BizTalk Application 1** nodo clic **puertos de recepción**y seleccione **puerto de recepción unidireccional / nueva**.  

12. En el **crear nuevo puerto de recepción** cuadro de diálogo el **especificar el tipo de puerto de recepción** cuadro, seleccione **puerto de recepción unidireccional** en la lista desplegable lista y, a continuación, haga clic en  **Aceptar**.  

13. En el **propiedades de puerto de recepción** cuadro de diálogo el **nombre** , escriba **AdapterReceive**y, a continuación, haga clic en **Aceptar**.  
...
16. En el **propiedades de ubicación de recepción** diálogo cuadro, realice lo siguiente.  


    |       Use       |                                                                                                                                                                                               Para                                                                                                                                                                                                |
    |----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    |       **Nombre**       |                                                                                                                                                                                     Tipo **AdapterReceiveLocation**                                                                                                                                                                                     |
...
-   En el Explorador de soluciones, haga clic en **AdapterHarnessProject**y, a continuación, haga clic en **recompilar**.  

##### <a name="to-deploy-the-adapter-harness-project"></a>Para implementar el proyecto de instrumento de adaptador  

1.  En el Explorador de soluciones, cuando se recompile el proyecto, haga clic en **AdapterHarnessProject**y, a continuación, haga clic en **implementar**.  
...
3. En el **propiedades de enlace de puertos: AdapterHarness.AdapterHarnessType: configuraciones de enlace** diálogo cuadro, realice lo siguiente.  
...
2.  Vaya a  *\<unidad\>*:**\Temp\Receive** y pegue el archivo Instance.xml en la carpeta.  
     ...
     Si la transmisión y recepción adaptadores están trabajando, debe mover el archivo de la  *\<unidad\>*:**\Temp\Receive** carpeta a la *\<unidad \>*:**\Temp\Send** carpeta.  
...
1. En [!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)], en el Explorador de soluciones, haga clic en **AdapterHarnessProject**, apunte a **agregar**y, a continuación, haga clic en **agregar elementos generados**.  

2. En el **agregar elementos generados - AdapterHarnessProject** cuadro de diálogo, haga clic en **agregar metadatos de adaptador**y, a continuación, haga clic en **abierto**.  
...
4. Expanda **organización de servicio**, **sanitaria**y, a continuación, haga clic en **administrativo**.  

    Tenga en cuenta que **elegibilidad** muestra de manera diferente de los demás nodos. **Elegibilidad** es un nodo de servicio que se puede seleccionar. El resto de nodos son nodos de organización que no describen ningún servicio específico.  
...
### <a name="comments"></a>Comentarios  
 Después de completar el adaptador de ejemplo, puede modificar el adaptador de ejemplo para crear un adaptador personalizado estático o dinámico para obtener más información, consulte [configuración de tiempo de diseño del adaptador](../core/adapter-design-time-configuration.md).  

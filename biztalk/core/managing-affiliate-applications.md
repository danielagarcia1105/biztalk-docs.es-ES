---
title: Administrar aplicaciones afiliadas | Documentos de Microsoft
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
helpviewer_keywords:
- managing [SSO applications]
- applications [SSO], managing
- SSO, applications
ms.assetid: 9bc6b120-6849-476a-b3af-c18bf2fa0197
caps.latest.revision: 8
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: dc5a355faed995fb6543b1e8f5810eec14584600
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 09/20/2017
ms.locfileid: "22262212"
---
# <a name="managing-affiliate-applications"></a>Administrar aplicaciones afiliadas
Esta sección proporciona información acerca de la creación y la configuración de aplicaciones afiliadas.  
  
## <a name="in-this-section"></a>En esta sección  
  
-   [Cómo crear una aplicación afiliada](../core/how-to-create-an-affiliate-application.md)  
  
-   [Cómo eliminar una aplicación afiliada](../core/how-to-delete-an-affiliate-application.md)  
  
-   [Cómo actualizar las propiedades de una aplicación afiliada](../core/how-to-update-the-properties-of-an-affiliate-application.md)  
  
-   [Cómo habilitar una aplicación afiliada](../core/how-to-enable-an-affiliate-application.md)  
  
-   [Cómo deshabilitar una aplicación afiliada](../core/how-to-disable-an-affiliate-application.md)  
  
-   [Cómo enumerar aplicaciones afiliadas](../core/how-to-list-affiliate-applications.md)  
  
-   [Cómo mostrar las propiedades de una aplicación afiliada](../core/how-to-list-the-properties-of-an-affiliate-application.md)  
  
-   [Cómo borrar la caché de aplicaciones](../core/how-to-clear-the-application-cache.md)  
  
-   [Cómo establecer el servidor SSO con la utilidad de cliente](../core/how-to-set-the-sso-server-using-the-client-utility.md)  
  
-   [Cómo mostrar el servidor SSO con la utilidad de cliente](../core/how-to-display-the-sso-server-using-the-client-utility.md)  
  
-   [Cómo establecer las credenciales para la aplicación afiliada con la utilidad de cliente](../core/how-to-set-credentials-for-the-affiliate-application-using-the-client-utility.md)
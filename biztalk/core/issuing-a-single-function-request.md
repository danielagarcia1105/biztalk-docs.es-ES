...
ms.locfileid: "22262036"
---
# <a name="issuing-a-single-function-request"></a>Emitir una solicitud de una sola función
El siguiente ejemplo, GetEffectiveAddress, es una llamada de una sola función a JD Edwards EnterpriseOne y, el resultado de esta solicitud, es un documento de respuesta estándar. En una solicitud de una sola función, sólo se especifica un callMethod en el objeto XML.  
  
## <a name="example-executing-a-business-function-with-a-single-function-call"></a>Ejemplo: Ejecutar una función empresarial con una llamada de función única  
 Lo que se muestra a continuación es un documento de ejemplo de solicitud GetEffectiveAddress. BizTalk Server lo genera como una instancia XML desde el XSD que se crea al agregar elementos generados. Este documento de ejemplo de solicitud se generó con el estilo de esquema ATTRIBUTE_STYLE de la configuración del puerto de envío.  
...
```  
 ...
 Lo que se muestra a continuación es una respuesta de ejemplo GetEffectiveAddress devuelta por J.D. Edwards EnterpriseOne como respuesta a la solicitud GetEffectiveAddress.  

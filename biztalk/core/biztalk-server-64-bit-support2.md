---
title: información general de la compatibilidad con 64 bits para que BizTalk Server | Microsoft Docs
description: compatibilidad con 64 bits en los adaptadores, los procesos, administración de BizTalk, las herramientas de BAM, ensamblados, orquestaciones etc. en BizTalk Server
ms.custom: ''
ms.date: 09/27/2017
...
ms.locfileid: "37013789"
---
# <a name="biztalk-server-64-bit-support"></a>Compatibilidad de BizTalk Server con 64 bits
En este tema se responde algunas preguntas más frecuentes sobre la compatibilidad de 64 bits de Microsoft [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)].  
  
## <a name="which-versions-of-64-bit-windows-are-supported"></a>¿Qué versiones de Windows de 64 bits son compatibles?  
 Todas las ediciones de [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] admiten la ejecución de 32 bits y la ejecución de 64 bits nativa en los sistemas operativos compatibles. [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] incluye opciones de configuración de 32 bits y 64 bits. 
...
## <a name="is-there-an-extra-cost-for-64-bit-support"></a>¿Supone la compatibilidad con 64 bits algún costo adicional?  
 No. compatibilidad con 64 bits se incluye sin costo adicional.  
...
##### <a name="biztalk-host-instances"></a>Instancias de host de BizTalk  
 Un “host” de BizTalk es un grupo lógico de servidores denominados “instancia de host”. Cada instancia de host se implementa como un servicio NT basado en BTSNTSvc.exe. Carga y ejecuta en instancias de host de las orquestaciones y adaptadores en curso. Las instancias de host pueden configurarse para ejecutarse en modo de 32 bits o 64 bits mediante el uso de la **solo 32 bits** opción de casilla de verificación en la **propiedades de Host** cuadro de diálogo en el [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] consola de administración.  
  
##### <a name="enterprise-sso"></a>SSO empresarial  
 El inicio de sesión único de Microsoft Enterprise Single Sign-On (SSO) se ejecuta en un servicio de NT dedicado (ENTSSO.exe). Es nativo de 32 bits en 32 bits [!INCLUDE[btsWinNoVersion](../includes/btswinnoversion-md.md)] y de 64 bits nativo en 64 bits [!INCLUDE[btsWinNoVersion](../includes/btswinnoversion-md.md)].  
 ...
 **Microsoft Visual Studio**  
 ...
 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] los ejecutables del diseñador están alojados en 32 bits [!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)] IDE. [!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)] admite el desarrollo de proyectos de 64 bits con Microsoft [!INCLUDE[btsDotNetFramework](../includes/btsdotnetframework-md.md)], que puede implementarse en [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)].  
 ...
 **Internet Explorer**  
 ...
 El cliente de BAM requiere Internet Explorer de 32 bits para instalarse y usarse en 64 bits [!INCLUDE[btsWinNoVersion](../includes/btswinnoversion-md.md)].  
...
## <a name="how-do-i-enable-native-64-bit-execution-of-orchestrations"></a>¿Cómo se habilita la ejecución de 64 bits de orquestaciones?  
 Asignar la orquestación que se ejecuta en una instancia de host que tiene el **solo 32 bits** propiedad no está seleccionada. La instancia de host se debe ejecutar en un equipo con Windows x64.  
...
## <a name="will-net-framework-20-compiled-assemblies-jit-compile-properly-in-both-32-bit-and-64-bit"></a>¿Los ensamblados compilados por .NET Framework 2.0 realizarán la compilación JIT adecuadamente tanto en sistemas de 32 bits como de 64?  
 Sí. Si el ensamblado se compiló con la [!INCLUDE[btsDotNetFramework](../includes/btsdotnetframework-md.md)] 2.0 y el **AnyCPU** marca un testamento DLL único a la compilación JIT adecuadamente en CLR de 32 bits o 64 bits.  
...
> 
> [!NOTE]
>  Si su [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] aplicaciones conectarse a un equipo de Windows Server 2003 (por ejemplo, SSO, SQL adaptador, MQSeries, MQSAgent, Oracle etc.) y volver a instalar [934016](http://support.microsoft.com/kb/934016) y [934849](http://support.microsoft.com/kb/934849) en estos servidores de Windows servidores de 2003.  
> 
> [!NOTE]
...
## <a name="are-persisted-biztalk-orchestrations-dependent-on-32-bit-or-64-bit-runtimes"></a>¿Son las orquestaciones guardadas de BizTalk dependientes de motores de tiempo de ejecución de 32 o 64 bits?  
 No. [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] continúa usando los formatos que son independientes de los tiempos de ejecución de 32 bits o 64 bits de componentes de tiempo de ejecución. Esto incluye orquestaciones, mensajes y puertos. Este modelo de persistencia permite a un administrador cambiar la configuración de host entre 32 y 64 bits sin crear incompatibilidades en [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] datos.  

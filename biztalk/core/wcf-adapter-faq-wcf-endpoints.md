---
title: 'P+F del adaptador WCF: Puntos de conexión WCF | Microsoft Docs'
ms.custom: ''
ms.date: 06/08/2017
...
ms.locfileid: "36980301"
---
# <a name="wcf-adapter-faq-wcf-endpoints"></a>P+F del adaptador WCF: Puntos de conexión WCF
...
## <a name="why-would-i-use-a-metadata-only-endpoint-in-biztalk-server"></a>¿Por qué debería usar un extremo de solo metadatos en BizTalk Server?  
 ...
 Si elige **publicar esquemas como servicio WCF**, el asistente le permite definir una definición de servicio mediante la especificación de nombres de servicio y la operación con esquemas de salida y entrada existente. El asistente crea un archivo .svc y un directorio virtual IIS para que se puedan examinar los metadatos después de publicarlos. El Asistente para publicación de servicio WCF de BizTalk también crea un archivo web.config con su atributo httpGetEnabled del \<serviceMetadata\> elemento establecido en true. Esto publica los metadatos para que se puedan examinar. Si elige publicar los metadatos del servicio de ubicación de recepción de BizTalk, esos datos pueden tener acceso a través de una solicitud GET a través de HTTP mediante un `?wsdl` al final de la dirección URL del servicio.  

---
title: "Recuperación ante desastres | Documentos de Microsoft"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: eefb036b-7254-407d-a203-66708c07f67d
caps.latest.revision: "2"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: ff6d7ba22180995d41fa536717804483931c7ad0
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 09/20/2017
---
# <a name="recovering-from-a-disaster"></a>Recuperarse de un desastre
En esta sección se detalla los pasos para transferir las operaciones de producción en el sitio de recuperación ante desastres. Para los fines de esta sección, el sistema de producción en línea se conoce como el sistema de "origen". El sitio de recuperación ante desastres se conoce como el sistema de "destino". Esta sección proporciona información general del proceso de recuperación ante desastres.  
  
## <a name="in-this-section"></a>En esta sección  
  
-   [Restaurar el grupo de BizTalk](../technical-guides/restoring-the-biztalk-group.md)  
  
-   [Recuperación de los equipos en tiempo de ejecución](../technical-guides/recovering-the-runtime-computers.md)  
  
-   [Recuperación de aplicaciones adicionales](../technical-guides/recovering-additional-applications.md)  
  
-   [Solución de problemas de trasvase de registros](../technical-guides/troubleshooting-log-shipping.md)
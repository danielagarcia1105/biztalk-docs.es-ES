...
---
# <a name="known-issues-with-certificates-in-biztalk-server"></a>Problemas conocidos con los certificados de servidor BizTalk Server
En esta sección se describe problemas conocidos con la administración de certificados digitales empleados con [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)].  
  
## <a name="general-certificate-issues"></a>Problemas generales de certificado  
  
### <a name="lack-of-connectivity-to-the-certificate-revocation-list-will-cause-a-certificate-to-be-rejected"></a>Falta de conectividad a la lista de revocación de certificados hará que un certificado que se rechacen  
 Este problema implica el siguiente error: "se produjo un error de autenticación. El estado de la entidad de certificación (CA) que emitió el certificado usado para firmar el mensaje es desconocido". Este error puede producirse cuando el certificado de firma es válido cuando se ven en MMC (mediante el [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] usuario) en el [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)].  
  
 Esta condición puede producirse si está habilitada la propiedad de "Comprobación de revocación de certificados" en el componente de descodificador de S/MIME en la canalización de recepción. Cuando esta propiedad está establecida en true, BizTalk Server intentará consultar la lista de revocación de certificados (CRL) para ver si se ha revocado el certificado entrante. No importa si no se ha revocado el certificado en Sí. Si BizTalk Server no se puede consultar la lista CRL correspondiente debido a problemas de conectividad, no acepta el certificado. Para solucionar este error, muestra las propiedades del certificado haciendo doble clic en el certificado que ha usado. En la ficha Detalles, verá el atributo "Punto de distribución de CRL" en la lista de campos. Debería haber varias direcciones URL en este atributo que apuntan a la CRL en el servidor de CA. El servidor BizTalk server debe poder tener acceso a cualquiera de estas direcciones URL para recuperar la CRL. En caso contrario, se producirá un error en la comprobación de revocación y se registrará el error anterior.  
  
### <a name="the-other-people-certificate-store-is-not-initialized-until-accessed"></a>No se ha inicializado el almacén de certificados otras personas hasta que se tiene acceso a  
...
> [!NOTE]
>  Puede modificar estos artefactos mediante la consola de administración si ha iniciado sesión directamente en el [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)].  
  
 En un equipo recién instalado, el **certificados otras personas** almacén no se ha inicializado a menos que se puede acceder a ella una vez. Durante la configuración del grupo, puede inicializar este **certificados otras personas** almacenar y así no ve este error en un equipo en el grupo al que se ha realizado la configuración.  
  
### <a name="biztalk-server-only-supports-one-personal-certificate-for-each-biztalk-group"></a>BizTalk Server solo admite un certificado personal para cada grupo de BizTalk  
...
### <a name="if-an-as2-message-cannot-be-decrypted-the-problem-may-be-fixed-by-re-importing-the-certificate"></a>Si no se puede descifrar un mensaje AS2, el problema puede solucionarse por volver a importar el certificado  
 **Síntoma**  
  
 El descodificador AS2 encuentra una excepción al intentar descifrar un mensaje AS2, generando el error siguiente:  
 ...
 **Resolución**  
 ...
 Elimine el certificado existente del almacén personal y, a continuación, vuelva a importar el certificado en el almacén personal mediante el Asistente para importación de certificados. Hacerlo haciendo clic con el **certificado** carpeta bajo la **Personal Store**, seleccionando **todas las tareas**y, a continuación, haga clic en **importación**.  
  
### <a name="use-the-same-logon-for-the-in-process-host-instance-and-the-isolated-host-instance-to-ensure-that-personal-store-is-recognized"></a>Use el mismo inicio de sesión para la instancia de host in-process y la instancia de host aislado para asegurarse de que se reconoce ese almacén personal  
...
2.  En el **ejecutar** cuadro de diálogo, escriba **service.msc** y presione **ENTRAR** para abrir el **servicios** complemento de MMC.  
  
3.  Abra el **propiedades** cuadro de diálogo para el servicio que creó. Haga clic en el servicio y seleccione **propiedades**.  
...
6.  Inicie manualmente el servicio para cargar el perfil de usuario para ese usuario de inicio de sesión.  
...
### <a name="the-key-usage-attribute-of-a-certificate-must-match-the-certificates-use"></a>El atributo Uso de la clave de un certificado debe coincidir con el uso del certificado.  
 Los certificados usados para el transporte AS2 deben tener los atributos necesarios para su uso previsto. Para la firma y la verificación de la firma, el atributo Uso de la clave del certificado debe ser Firma digital. Para el cifrado y el descifrado, el atributo Uso de la clave del certificado debe ser Cifrado de datos o Cifrado de clave. Puede comprobar el atributo uso de la clave haciendo doble clic en el certificado, haga clic en el **detalles** pestaña en el **certificado** cuadro de diálogo y comprobación de la **uso clave** campo .  
